...
class VideoPanoRefTrainData(data.Dataset):
    def __init__(self, opt):
        super(VideoPanoRefTrainData, self).__init__()
        ...
        self.video_gt.set(cv2.CAP_PROP_POS_FRAMES, 1)
        success, self.frame_ref = self.video_gt.read()
    ...
    def __getitem__(self, idx):
        ...
        image_ref = crop_panorama_image(self.frame_ref, theta, phi, size=self.size_gt, fov=self.fov)
        ...
        image_ref = cv2.cvtColor(image_ref, cv2.COLOR_RGB2BGR)
        ...
        ref = ToTensor()(image_ref.copy())
        lq = ToTensor()(image_lq.copy())
        gt = ToTensor()(image_gt.copy())
        return {'gt': gt, 'lq': lq, 'ref': ref, 'name': filename}
...
@DATASET_REGISTRY.register()
class VideoPanoRefTestData(data.Dataset):
    def __init__(self, opt):
        super(VideoPanoRefTestData, self).__init__()
        self.batch = 8 
        self.video_gt = cv2.VideoCapture(opt['dataroot_gt'])
        self.video_lq = cv2.VideoCapture(opt['dataroot_lq'])
        self.count = self.video_gt.get(cv2.CAP_PROP_FRAME_COUNT)
