# Modified from https://github.com/lbc12345/SeD/blob/main/models/sed.py
...
class CLIPSemanticExtractor(ModifiedResNet):
    def __init__(self, layers=(3, 4, 6, 3), pretrained=True, path=None, output_dim=1024, heads=32):
        super(CLIPSemanticExtractor, self).__init__(layers=layers, output_dim=output_dim, heads=heads)

        ckpt = 'RN50' if path is None else path
        if pretrained:
            model, _ = load(ckpt, device='cpu')
...
class DiscriminatorSeDP(nn.Module):
    def __init__(self, input_nc, ndf=64, semantic_dim=1024, semantic_size=16, use_bias=True, nheads=1, dhead=64, path=None):
        ...
        """
        super(DiscriminatorSeDP, self).__init__()

        self.model_ex = CLIPSemanticExtractor(path=path)
        kw = 4
        padw = 1        
    ...
    def forward(self, sr, hr):
        semantic = self.model_ex(hr)
        """Standard forward."""
        input = self.conv_first(sr)
...
@ARCH_REGISTRY.register()
class DiscriminatorSeDU(nn.Module):
    
    def __init__(self, num_in_ch=3, num_feat=64, semantic_dim=1024, semantic_size=16, skip_connection=True, nheads=1, dhead=64, path=None):
        super(DiscriminatorSeDU, self).__init__()
    ...
    def forward(self, sr, hr):
        semantic = self.model_ex(hr)
        # downsample
        x0 = F.leaky_relu(self.conv0(sr), negative_slope=0.2, inplace=True)
